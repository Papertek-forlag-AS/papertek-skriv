...
| `paragraph-trainer-route.js` | `renderParagraphTrainerScreen`             | editor-core/student/paragraph-trainer, i18n, html-escape, school-level | Route + screen wiring for `#/avsnitt`; hosts the portable three-step paragraph trainer and passes `getLevel` (school level) into it. Writes nothing to IndexedDB — attempts live in localStorage inside the trainer |
| `stabekk-page.js`      | (self-executing)                                 | i18n, theme, toast-notification, sw-manager, editor-core/student/paragraph-trainer | Entry point for `stabekk.html` — standalone Stabekk one-pager hosting the paragraph trainer without the Skriv shell (no router/onboarding). Fixed school level via `STABEKK_LEVEL` constant; header has its own theme-cycle button (standalone visitors can't reach Skriv's). Stabekk accent palette comes from the page's Tailwind config (emerald scale remap anchored in Pantone 640 C, Akershus fylkeskommune's main color), the trainer module is reused unchanged |
