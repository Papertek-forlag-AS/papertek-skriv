...
| `leksihjelp-settings.js` | `initLeksihjelpSettings` | i18n, HTML escaping, runtime Leksihjelp data | Slide-in drawer: a debounced (120 ms) bidirectional dictionary search (word or translation, top 8 results), Eksamensmodus toggle, Skrivespråk/Oppslagsspråk pickers, and a Grammatikknivå section built on the vendored `grammar-features-section.js` with preset pills (Lite/Middels/Mye/Alt); hidden when the bridge reports `status === 'extension'` |
...
| `js/leksihjelp/**` | generated `window.__lexi*` globals | loader shim and upstream-manifest-ordered sibling scripts | Vendored Leksihjelp 3.8.127 snapshot (commit `40241513b0995dfb7f482e745cb771e74204a96b`); generated by `scripts/sync-leksihjelp.js`, never hand-edited. An earlier draft of this row said 3.8.87 — that version is stale; `public/js/leksihjelp/.version` is authoritative |
