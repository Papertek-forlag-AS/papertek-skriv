 ...
 *
 * Version snapshots are retained for soft deletes and removed only when the
 * corresponding document is permanently deleted.
 *
 * Usage:
...
import { deleteSnapshotsForDocuments } from '../editor-core/student/version-history.js';
...
export async function permanentlyDelete(id) {
    const db = await openSkrivDatabase();
    await deleteSnapshotsForDocuments([id]);
...
export async function emptyTrash() {
    ...
    await deleteSnapshotsForDocuments(trashedDocs.map(doc => doc.id));
...
export async function purgeExpired() {
    ...
    await deleteSnapshotsForDocuments(expired.map(doc => doc.id));
